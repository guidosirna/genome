	...
	var	ECT = function (options) {
		...
		var
			...
			compile = function (template) {
				...
				var tpl = '(function __ectTemplate(__ectTemplateContext, __ectFileInfo, include, content, block) {\n' + CoffeeScript.compile(buffer, { bare : true }) + '});';

				if(!ect.options.keepSpaces) {
					tpl = tpl.replace(/(\r\n|\n|\r|\t|\\n|\\t)/gm, '').replace(/\s+/gm, ' ')
				}
		...
		this.setCompiledTemplate = function(name, template) {
			cache[name] = { file: name, compiled: template }
		};
